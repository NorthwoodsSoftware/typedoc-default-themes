...
(function (typedoc) {
    ...
    var FilterItemCheckbox = (function (_super) {
        ...
        FilterItemCheckbox.prototype.initialize = function () {
            var _this = this;
            typedoc.$html.toggleClass('toggle-' + this.key, !this.defaultValue);
        ...
        FilterItemCheckbox.prototype.handleValueChange = function (oldValue, newValue) {
            ...
            typedoc.$html.toggleClass('toggle-' + this.key, !newValue);
        };
    ...
    var FilterItemSelect = (function (_super) {
        ...
        FilterItemSelect.prototype.initialize = function () {
            var _this = this;
            typedoc.$html.addClass('toggle-' + this.value);
    ...
    var Filter = (function (_super) {
        ...
        function Filter(options) {
            ...
            var visibilityDefault = $('tsd-filter-visibility').find('li.selected').attr('data-value');
            var inheritedDefault = $('#tsd-filter-inherited').prop('checked');
            var externalsDefault = $('#tsd-filter-externals').prop('checked');
            var onlyExportedDefault = $('#tsd-filter-only-exported').prop('checked');
            if (visibilityDefault === undefined)
                visibilityDefault = 'private';
            if (inheritedDefault === undefined)
                inheritedDefault = 'private';
            if (externalsDefault === undefined)
                externalsDefault = 'private';
            if (onlyExportedDefault === undefined)
                onlyExportedDefault = 'private';
            _this.optionVisibility = new FilterItemSelect('visibility', visibilityDefault);
            _this.optionInherited = new FilterItemCheckbox('inherited', inheritedDefault);
            _this.optionExternals = new FilterItemCheckbox('externals', externalsDefault);
            _this.optionOnlyExported = new FilterItemCheckbox('only-exported', onlyExportedDefault);
            return _this;
        }
    ...
    (function (search) {
        ...
        function updateResults() {
            ...
            for (var i = 0, c = 0; i < res.length && c < 10; i++) {
                var row = search.data[Number(res[i].ref)];
                if (!(typedoc.$html.hasClass('toggle-inherited') && row.classes.indexOf('tsd-is-inherited') >= 0)) {
                    var name = row.fullName.replace(new RegExp(query, 'i'), function (match) { return "<span class=\"match\">" + match + "</span>"; });
